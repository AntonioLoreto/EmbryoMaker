...
subroutine trait_distancenormalization(fitelli)
   Implicit none
   ...
   real*8, dimension(26)   :: corners
   real*8, dimension(26,3) :: cornerscoord
   real*8, dimension(26)   :: target_corners,distances,target_distances,diff_distances
   real, allocatable, dimension(:,:)      :: ncoords
   integer, allocatable, dimension(:,:,:) :: bfillz 
   !******************************************************************************************************!
   call iniboxes
   ...
   sumdist = 0
   do ihc=1,26
      if(target_distances(ihc) == 0)then; cycle; endif
      distances(ihc) = sqrt((cornerscoord(ihc,1))**2+(cornerscoord(ihc,2))**2+(cornerscoord(ihc,3))**2)
   ...
   do ihc=1,26
      if(target_distances(ihc) == 0)then; cycle; endif
      diff_distances(ihc) = abs(target_distances(ihc)- distances(ihc))
   enddo
