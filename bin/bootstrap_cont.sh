#!/bin/bash
#AL 7-3-34>> all comments by Antonio Loreto 
usage="
...
startfile=$(readlink -f $1) #first argument is the .dat morphology initial conditions file
workdir=$(readlink -f $2) #second argument is direction of bin
nsize=$3 # population number
ncpus=$4 # Number of CPUs that will run development of individuals. 
generation=0
...
echo "substitution   individual   parent_subs parent   fitness   EMD    Symmetry_robustness   mutation   geni   genj   prevalue   newvalue time" > ${workdir}/output_model.dat
...
for (( i = 1; i <= ncpus; i++ )); do
    ...
    cp $workdir/ranges.dat $individual/ranges.dat   # HC 11-11-2021 #AL 7-3-24>> this could be avoided because all individuals use the same range of paramaters so copying them in each ind. folder is kind of dumb
...
timeout 3000 $seeding $workdir $startfile $ncpus $nsize #this means to kill after 3000 seconds.
