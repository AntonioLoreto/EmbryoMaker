program substitute
   ...
   integer         :: donorsubnum, errorr,inter,waittime
   ...
   if(nsubs>waittime)then                                       !!>> AL 24-3-25 we end simulation if no increase in fitness during arbitrary number of subs (we reached saturation)
      
      ! System command to get the latest modified file in the folder
      filename = trim(bindir)//"/best/"
      command = "ls -t "//trim(filename)//" | head -n 1 > latest_file.txt"
      
      call system(trim(command))

      ! Open the file containing the latest file name
      ...
      if((nsubs - numm) > 50000) nsubs = nmax
   ...
   if (nsubs .le. nmax) then                                !!>> HC 21-11-2020 IF there are still substitutions to do  
     ...
     ! call system(trim(cp))                                     
      ...
      !print*,"max distance in population:",maxfit
