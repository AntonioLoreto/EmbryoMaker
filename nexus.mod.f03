...
module nexus
  ...
  use conservative_R !AL>>23-05-24
...
subroutine filter  !!>> HC 12-2-2021 This subroutine applies filters if the corresponding ffu(21) is ON
  ...
  integer ::cuantosnd
     ...
     ! WHICHEND 11: if== 1 -> NO MOVEMENT                                                                                          !!>>AL 15-6-24
    ...
    if (ffufi(11,1)==3)then                                                                                     
      dieornot = 0       
      if(nd >= 200) then !AL >> 3-07-24: this is arbitrary; heuristically, big morphogenesis starts at this size (or not, it depends on dev. mechanisms really...)
        inquire(file="prueba_ass.txt", exist=exist)
        !print*,"numero de nodos: ", nd
        if(exist)then
          open(6021, file="prueba_ass.txt")
            read(6021, *) cuantosnd
          close(6021)
          if((nd - (cuantosnd+ffufi(11,3))) >= 0)then
            open(6021, file="prueba_ass.txt", action="write")
              write(6021, *) nd
            close(6021)
        ...
        if (dieornot == 1) then
          ...
          open(891, file='individual.datfitness')                         !!>> AL 2-07-24 We want to know
            write(891,*) 0.000666                                         !!>> AL 2-07-24 why we killed this indv
          close(891)   
          goto 171                                                                      
        end if
      end if
...
!     endif                                                                                                      !!>> HC 17-9-2020
    ...
    !WHICHEND 15: ASSYMETRY IS TO BIG (>5%) (GETOT)                                                            !!>> AL 2-07-24
    !getot = numero de iteraciones
    !if (ffufi(15,1)==1) then                                                                               
    !  if (mod(getot,ffufi(15,3)).eq.0) then   
    !      call assymetry(dieornot)                                                                            
    !      if (dieornot == 1) then
    !        whichend=15                                                                                                   
    ...
    !if (ffufi(15,1)==1) then
    !  dieornot = 0       
    !  if(nd >= 12000) then !AL >> 3-07-24: this is arbitrary; heuristically, big morphogenesis starts at this size (or not, it depends on dev. mechanisms really...)
    !    inquire(file="prueba_ass.txt", exist=exist)
    !    print*,"numero de nodos: ", nd
    !    if (exist) then
    !      open(6021, file="prueba_ass.txt")
    !        read(6021, *) cuantosnd
    !      close(6021)
    !      print*,"cuantosnd",cuantosnd
    !      print*,"cuantosnd+ffufi(15,3)", cuantosnd+ffufi(15,3)
    !      if(cuantosnd+ffufi(15,3)-10 <= nd .and. nd <= cuantosnd+ffufi(15,3)+10 )then
    !        open(6021, file="prueba_ass.txt", action="write")
    !          write(6021, *) nd
    !        close(6021)
    !        call assymetry(dieornot) 
...
! 15 = ASSYMETRY                                         !!>> AL 23-5-2024


  !carg = fichero input EM
